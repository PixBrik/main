...
export function LibraryScreen({
  ...
  const [entries]=useState<LibraryEntry[]>(()=>listLibrary().filter(isLibraryEntryReleased));
  ...
  const canBuild=!!selected&&(!!selected.proceduralKey||!!selected.meshUrl)&&(!selected.supportsHolder||!!sanitiseMessage(message));
  const requestBuild=()=>{
  ...
  return <ScreenFrame accent="indigo" eyebrow="Library / Browse and customise" navigationDisabled={generating} onBack={onBack}
    ...
    footer={<PrimaryButton disabled={!canBuild||generating} label={generating?`Building three sizes... ${Math.round(generationProgress*100)}%`:selected?`Build ${selected.name}`:'Choose an object to customise'} onPress={requestBuild}/>}>
    ...
    {category==='message'?<View style={styles.messageIntro}><Text style={styles.kicker}>CREATE FROM SCRATCH</Text><Text style={styles.messageIntroTitle}>Your words, built in bricks.</Text><Text style={styles.hint}>Up to 14 Latin letters, numbers and spaces. Longer messages wrap automatically. Pick the mounting style below.</Text></View>:
      visible.length?<View style={styles.grid}>{visible.map(entry=>{const active=entry.id===selectedId;return <Pressable accessibilityRole="button" accessibilityState={{selected:active}} key={entry.id} onPress={()=>select(entry)} style={[styles.card,active&&styles.cardActive]}><View style={styles.thumbnail}><BrickThumbnail color={entry.defaultColor} entry={entry} options={{font:'block',holder:'flat',message:entry.presetMessage}}/></View><View style={styles.cardBody}><Text numberOfLines={2} style={styles.cardName}>{entry.name}</Text><Text style={styles.cardMeta}>{entry.theme?.replace('-', ' ')??entry.category}</Text></View></Pressable>;})}</View>:
